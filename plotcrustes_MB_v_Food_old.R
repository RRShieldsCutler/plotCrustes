...
unw_A <- read.delim('../IMP/data/unweighted_unifrac_dm.txt',
                    header=1, row.names = 1, check.names = F)
unw_B <- read.delim('../IMP/data/uwuf_food_dm.txt',
                    header=1, row.names = 1, check.names = F)
...
meta <- read.delim('../IMP/data/SampleIDs-CS.txt',
...
rownames(unw_A) <- unlist(lapply(X=rownames(unw_A), FUN = function(xx) paste0(xx, '_microbiome'))) 
colnames(unw_A) <- unlist(lapply(X=colnames(unw_A), FUN = function(xx) paste0(xx, '_microbiome')))
rownames(unw_B) <- unlist(lapply(X=rownames(unw_B), FUN = function(xx) paste0(xx, '_food')))
colnames(unw_B) <- unlist(lapply(X=colnames(unw_B), FUN = function(xx) paste0(xx, '_food')))
...
A_crust$data_type <- 'microbiome'; B_crust$data_type <- 'food'
...
for (j in 1:9) { 
  ...
  Ap_crust$data_type <- 'microbiome'; Bp_crust$data_type <- 'food'
...
meta.big <- read.delim('../IMP/data/mappingIMP.txt', header=1, sep = '\t', check.names = F)
meta.big <- meta.big[meta.big$`#SampleID` %in% as.character(sample_ids), ]

dist_plot$sampleID <- unlist(lapply(X=dist_plot$sampleID_A,
...
# Set of typical procrustes plots WITH GROUPS
PCOA_plot_many.groups <- merge(PCOA_plot_many, meta.big[,c('#SampleID','Sample.Group')], by.x = 'participant', by.y='#SampleID')
PCOA_plot_many.groups$Sample.Group <- factor(PCOA_plot_many.groups$Sample.Group,
                                             levels = c("KarenThai", "HmongThai", "Karen1st",  "Hmong1st",  "Hmong2nd",  "Control"),
                                             ordered = T)
...
PCOA_plot_many.groups.sub <- PCOA_plot_many.groups[PCOA_plot_many.groups$realperm == 'permuted2' | PCOA_plot_many.groups$realperm == 'real', ]

pscat.groups <- ggplot(PCOA_plot_many.groups.sub) +
  geom_line(aes(x=PC1, y=PC2, group=participant, color=Sample.Group), alpha=0.2) +
  geom_point(aes(x=PC1, y=PC2, color=Sample.Group, shape=data_type), size=1, alpha=0.7) +
  
  theme_classic() + facet_grid(. ~ realperm) +
  theme(axis.text = element_text(color='black', size = 6)) + coord_fixed(ratio = 1) +
  scale_color_manual(values = cols) + theme(legend.title=element_blank())
# To save
ggsave(pscat.groups, filename = 'procrustes_vs_med_permuted_group_colored_v2.png', width = 8.5, height = 4, dpi = 300)


dist_plot_groups.sub <- dist_plot_groups[dist_plot_groups$real_perm == 'permuted1' | dist_plot_groups$real_perm == 'true_distance', ]
...
# 
# ##Testing permuting the A category not B
# real_dist$real_perm <- 'true_distance'
# dist_plot <- real_dist
# PCOA_plot_many <- rbind(A_crust, B_crust)  # Start with the real data, then add the permutations
# procrust_dists <- vector(mode='numeric', length=9)
...
#   metaAp <- metaA
#   set.seed(j)
#   metaAp[,thing] <- sample(x = metaAp[,thing], size = length(metaAp[,thing]), replace = F)
#   metaAp <- metaAp[order(metaAp[,thing]),]
#   unw_Ap <- unw_A[rownames(metaAp),rownames(metaAp)]
#   pcoa_Ap <- pcoa(unw_Ap)$vectors
#   for(c in 1:ncol(pcoa_Ap)){
#     colnames(pcoa_Ap)[c] <- paste0("PC",c)
#   }
#   set.seed(j)
#   crusty_p <- procrustes(pcoa_Ap, pcoa_B, symmetric = T)
#   # crust_test_perm_p <- protest(pcoa_A, pcoa_Bp, permutations = how(nperm = 999))$signif
#   # cat(paste0(crust_test_perm_p, '\n'))
#   Ap_crust <- data.frame(crusty_p$X)
#   Bp_crust <- data.frame(crusty_p$Yrot)
